...
export default function EditCrewPage({
  ...
  useEffect(() => {
    async function loadCrew() {
      try {
        setLoadingCrew(true);
        const res = await fetch(`/api/tasks/crews/${id}`);
        const json = await res.json();
        if (!res.ok || !json.ok) {
          throw new Error(json.error || "加载失败");
        }
        const crew = json.data;
        setName(crew.name);
        setManagerId(crew.managerId ?? "");
        setTaskForms(
          crew.tasks.length > 0
            ? crew.tasks.map(
                (t: {
        ...
        const message = e instanceof Error ? e.message : "加载团队任务失败";
        setError(message);
      } finally {
              ...
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap gap-2">
                  {taskForms
                    ...
                    .map((other) => {
                      const isChecked = task.contexts.includes(other.id);
                      return (
                        <label
                          key={other.key}
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              const next = e.target.checked
                                ? [...task.contexts, other.id]
                                : task.contexts.filter((cid) => cid !== other.id);
                  ...
                  {taskForms.filter((t) => t.key !== task.key && t.id).length ===
                    0 && (
